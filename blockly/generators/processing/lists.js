...
/**
 * @fileoverview Generating Processing for list blocks.
 * @author fraser@google.com (Neil Fraser)
 */
'use strict';

goog.provide('Blockly.Processing.lists');

goog.require('Blockly.Processing');


Blockly.Processing['lists_create_empty'] = function(block) {
  // Create an empty list.
  return ['[]', Blockly.Processing.ORDER_ATOMIC];
};

Blockly.Processing['lists_create_with'] = function(block) {
...
Blockly.Processing['lists_repeat'] = function(block) {
  ...
  var functionName = Blockly.Processing.provideFunction_(
      ...
      ['function ' + Blockly.Processing.FUNCTION_NAME_PLACEHOLDER_ +
       ...
       '  return array;',
       '}']);
  var element = Blockly.Processing.valueToCode(block, 'ITEM',
      Blockly.Processing.ORDER_COMMA) || 'null';
  var repeatCount = Blockly.Processing.valueToCode(block, 'NUM',
...
Blockly.Processing['lists_length'] = function(block) {
  ...
  return [list + '.length', Blockly.Processing.ORDER_MEMBER];
};

Blockly.Processing['lists_isEmpty'] = function(block) {
  // Is the string null or array empty?
  var list = Blockly.Processing.valueToCode(block, 'VALUE',
      Blockly.Processing.ORDER_MEMBER) || '[]';
  return ['!' + list + '.length', Blockly.Processing.ORDER_LOGICAL_NOT];
};

Blockly.Processing['lists_indexOf'] = function(block) {
  // Find an item in the list.
  var operator = block.getFieldValue('END') == 'FIRST' ?
      'indexOf' : 'lastIndexOf';
  var item = Blockly.Processing.valueToCode(block, 'FIND',
...
Blockly.Processing['lists_getIndex'] = function(block) {
  // Get element at index.
  // Note: Until January 2013 this block did not have MODE or WHERE inputs.
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var listOrder = (where == 'RANDOM') ? Blockly.Processing.ORDER_COMMA :
      Blockly.Processing.ORDER_MEMBER;
  var list = Blockly.Processing.valueToCode(block, 'VALUE', listOrder) || '[]';
...
Blockly.Processing['lists_setIndex'] = function(block) {
  ...
  function cacheList() {
    ...
    var listVar = Blockly.Processing.variableDB_.getDistinctName(
  ...
  switch (where) {
    ...
    case ('FROM_START'):
      ...
      break;
    case ('FROM_END'):
      var at = Blockly.Processing.getAdjusted(block, 'AT', 1, false,
    ...
    case ('RANDOM'):
      var code = cacheList();
      var xVar = Blockly.Processing.variableDB_.getDistinctName(
 ...
 */
Blockly.Processing.lists.getIndex_ = function(listName, where, opt_at) {
  if (where == 'FIRST') {
    return '0';
...
Blockly.Processing['lists_getSublist'] = function(block) {
  // Get sublist.
  var list = Blockly.Processing.valueToCode(block, 'LIST',
      Blockly.Processing.ORDER_MEMBER) || '[]';
  ...
  if (where1 == 'FIRST' && where2 == 'LAST') {
    ...
    var getIndex_ = Blockly.Processing.lists.getIndex_;
...
Blockly.Processing['lists_sort'] = function(block) {
  // Block for sorting a list.
  var list = Blockly.Processing.valueToCode(block, 'LIST',
      Blockly.Processing.ORDER_FUNCTION_CALL) || '[]';
  var direction = block.getFieldValue('DIRECTION') === '1' ? 1 : -1;
  var type = block.getFieldValue('TYPE');
  var getCompareFunctionName = Blockly.Processing.provideFunction_(
      'listsGetSortCompare',
      ['function ' + Blockly.Processing.FUNCTION_NAME_PLACEHOLDER_ +
          '(type, direction) {',
       '  var compareFuncs = {',
...
Blockly.Processing['lists_split'] = function(block) {
  // Block for splitting text into a list, or joining a list into text.
  var input = Blockly.Processing.valueToCode(block, 'INPUT',
      Blockly.Processing.ORDER_MEMBER);
  var delimiter = Blockly.Processing.valueToCode(block, 'DELIM',
      Blockly.Processing.ORDER_NONE) || '\'\'';
...
Blockly.Processing['lists_reverse'] = function(block) {
