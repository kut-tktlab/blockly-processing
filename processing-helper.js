(function(global) {
  ...
  global.runSketch = function(callback) {
    try {
      ...
      output.value = '';
      canvas = createCanvas();
      var sketch = Processing.compile(global.sketchCode);

      if (callback) {
        if (!/exit\(\);/.test(global.sketchCode)) {
  ...
  global.convertToJS = function() {
    try {
      output.value = js_beautify(Processing.compile(global.sketchCode).sourceCode).replace(/\n\n\n+/g, '\n\n');
      var l, last, count=5;
